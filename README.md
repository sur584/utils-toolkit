...
- **修复** 依赖不可安装问题，补全版本上界防依赖漂移，拆分可选依赖，安全升级 postcss
- **修复** 前端 Promise 错误处理，补充缺陷分析与优化报告文档

### 2026-07-27
- **新增** 视频文案提取支持多文件批量上传
- **修复** Twitter/X 视频解析与下载（直连优先、代理兜底）

### 2026-07-16
- **修复** 抖音主页视频下载带水印问题
- **新增** 下载缓存定时清理（每 30 分钟清理超 6 小时旧文件）

### 2026-07-15
- **新增** 视频文案提取支持 TXT / SRT 双格式导出，启动自动检测 GPU（CUDA）
- **新增** 批量解析加序号、下载失败可重试、502 浏览器直连兜底
- **新增** 小红书主页批量解析（登录 Cookie + 分页抓全）
- **修复** 抖音单视频解析改走官方 API，主页分页每页条数一致、支持页码跳转

### 2026-07-14
- **新增** 博主主页解析分页浏览 + 单页打包 ZIP
- **新增** 单视频下载可中途取消，yt-dlp 下载失败自动重试

### 2026-06-25
- **修复** 跨设备访问 ASGI 响应错误，代理自动检测支持 IPv6 映射地址

### 2026-06-23
- **新增** 代理自动检测，局域网用户无需手动配置代理
- **修复** Twitter/X、TikTok 视频解析

### 2026-06-17
- **新增** `update.bat` 在线更新脚本（无需 git，一键拉取最新版）
- **优化** TikTok 解析：自动检测系统代理 + yt-dlp 双路解析

### 2026-06-11
- **新增** 视频号解析工具
- **优化** 全局主题统一，智能抠图新增撤销重做（Ctrl+Z/Y）与视图平移

### 2026-06-24
- **优化** 视频解析下载：修复 TikTok 批量解析回退、长标题 Windows 下载失败和重复点击多次下载问题
- **优化** 图片批量处理：压缩后可继续去水印/高清化，增强右下角水印去除和结果预览体验
